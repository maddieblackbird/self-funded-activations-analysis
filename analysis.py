...
if len(spend_activations) == 0:
    ...
    empty_df = pd.DataFrame(columns=[
        'activation_id', 'restaurant_name', 'location_name', 'activation_description',
...
for idx, activation in spend_activations.iterrows():
    ...
    # Match transactions during activation period
    matching_txns = transactions[
        (transactions['match_key'] == match_key) &
        (transactions['created_at_dt'] >= start_dt) &
        (transactions['created_at_dt'] <= end_dt)
    ].copy()
    ...
    activation_start_time = start_dt.time()
    activation_end_time = end_dt.time()
    activation_dow = start_dt.weekday()  # Monday=0
    ...
    for week_offset in range(1, 5):  # Previous 4 weeks
        baseline_start = start_dt - timedelta(weeks=week_offset)
        baseline_end = end_dt - timedelta(weeks=week_offset)
